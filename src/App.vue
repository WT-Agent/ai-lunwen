  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">学位层次</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">学科方向</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">学术论文写作规范与论文降重指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 学术论文大纲与写作指导专家');
...
const metricsList = [
  {
    "key": "academicHierarchyLogic",
    "label": "学术结构逻辑严密性"
  },
  {
    "key": "literatureSynthesisDepth",
    "label": "文献综述融会贯通度"
  },
  {
    "key": "methodologicalRigor",
    "label": "研究方法严谨度"
  },
  {
    "key": "academicToneStandard",
    "label": "学术规范语体度"
  },
  {
    "key": "thesisDefenseReadiness",
    "label": "答辩应对准备度"
  }
];
const guideRules = [
  {
    "title": "大纲三层递进",
    "advice": "大纲结构遵循“提出问题-分析问题-解决问题”逻辑，章与章之间层层递进",
    "avoid": "切忌大纲章节标题重复或逻辑跳跃"
  },
  {
    "title": "文献综述述评结合",
    "advice": "文献综述必须有“述”有“评”，在归纳前人研究后指出当前研究的空白与不足",
    "avoid": "严禁将文献综述写成文献摘要的简单罗列"
  },
  {
    "title": "学术语体客观性",
    "advice": "论文写作使用第三人称，语言保持中立、严谨、客观，多用实验数据与事实说话",
    "avoid": "避免在论文中使用“我认为”、“我觉得”等主观感性词汇"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[LUNWEN_SCORES\][\s\S]*?\[\/LUNWEN_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[LUNWEN_SCORES\](.*?)\[\/LUNWEN_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('lunwen_history_records');
...
const saveHistory = () => {
  localStorage.setItem('lunwen_history_records', JSON.stringify(historyList.value));
};
